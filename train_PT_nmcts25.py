...
from g_reward import *

parser = argparse.ArgumentParser()

""" Hyperparameter"""
parser.add_argument("--n_playout", type=int, default=25)
...
path = '/home/hail/PreferenceTransformer/JaxPref/reward_model/chess/PrefTransformer/Classical_expert_low/s7/'
...
def self_play(env, mcts_player, temp, game_iter=0, self_play_i=0):
    ...
    while True:
        ...
        if obs_history is None:
            obs_history = obs_[np.newaxis, ...]  # Add a new axis for the first observation
        else:
            obs_history = np.concatenate((obs_history, obs_[np.newaxis, ...]), axis=0)  # S
        ...
        if env.terminal is True:
            pt_reward = reward_from_preference_transformer(obs_history, move_history, reward_model)

            wandb.log({"selfplay/pt_reward": pt_reward,
                       "selfplay/alphazero_reward": env.reward,
            ...
            if env.reward != 0:  # non draw
                if env.reward == -1:
                    env.reward = 0
                winners_z[np.array(current_player) == 1 - env.reward] = pt_reward
                winners_z[np.array(current_player) != 1 - env.reward] = -pt_reward
                if env.reward == 0:
                    env.reward = -1
...
if __name__ == '__main__':
    ...
    env = Chess()
    state = env.reset()
    reward_model = initialize_model(path)
    ...
    if init_model:
        ...
        policy_value_net = PolicyValueNet(state.shape[0], state.shape[1], device)     # 8 by 8
